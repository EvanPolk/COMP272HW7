...
public class ProblemSolutions {
        ...
        for (int i = 0; i < n - 1; i++) {
            for (int j = i; j < n; j++) {
                if (ascending && values[i] > values[j]) {
                    ...
                    values[i] = values[j];
                    values[j] = temp;
                } else if (!ascending && values[i] < values[j]) {
                    int temp = values[j];
                    values[j] = values[i];
                    values[i] = temp;
                }
    ...
    private void mergeDivisbleByKFirst(int arr[], int k, int left, int mid, int right) {
        int[] partitionLeft = Arrays.copyOfRange(arr, left, mid + 1);
        int[] partitionRight = Arrays.copyOfRange(arr, mid + 1, right + 1);

        int l = 0, r = 0;
        while (l < partitionLeft.length && r < partitionRight.length) {
            if (partitionLeft[l] % k == 0) {
                arr[left++] = partitionLeft[l++];
            } else if (partitionRight[r] % k == 0) {
                arr[left++] = partitionRight[r++];
            } else if (partitionLeft[l] < partitionRight[r]) {
                arr[left++] = partitionLeft[l++];
            } else if (partitionLeft[l] >= partitionRight[r]) {
                arr[left++] = partitionRight[r++];
            }
        ...
        while (l < partitionLeft.length) {
            arr[left++] = partitionLeft[l++];
        }
        while (r < partitionRight.length) {
            arr[left++] = partitionRight[r++];
        }
    }
    ...
    public static int numRescueSleds(int[] people, int limit) {
        ...
        while (l < r) {
            ...
            // r is too big to join another person, so we send them
            if (people[l] + people[r] > limit) {
                sleds += 1;
                r--;
            }

            // Good to go, best combo we can get
            if (people[l] + people[r] <= limit) {
                sleds += 1;
                l++;
                r--;
        ...
        // Edge case where pointers end on one person, who is sent alone
        if (l == r) {
            sleds += 1;
        }
        return sleds;
